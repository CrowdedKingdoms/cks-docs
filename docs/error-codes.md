...
| `ENGINE_SWITCHED_OFF` | A legacy developer-code engine (game-model functions and automations, compute modules, player compute) is switched off on this tier, and the request would run or compile code on it. 503. Reads, deletes and diagnostics still answer. On dev these engines are off: ck-exec replaces them. | Move the code to [ck-exec](/exec/intro); [from the legacy engines](/exec/from-the-legacy-engines) maps each API to its replacement. Not retryable on this tier. |
