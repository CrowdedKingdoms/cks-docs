...
- **Agentic Crowdy Studio (allowlisted)** — the DeepSeek Harness agent running
  ...
  and billed to the player's wallet by default. Access remains allowlisted per
  app. See [Agentic Crowdy Studio](/crowdyjs/agentic-crowdy-studio).

Sign-in uses `client.auth`: **`login` / `register`** (email + password), magic link, or social/OIDC, and returns an **identity session token** (Management API only — account, studio admin, and minting app tokens); gameplay uses a short-lived **app-scoped token** confined to one app. `client.portal` mints app tokens (with a consent gate for untrusted apps), and the **two-client pattern** keeps an identity client (session token) separate from a per-game client (app token). See **[Best practices](/crowdyjs/best-practices)**, **[Portals & app-scoped tokens](/management-api/portals-and-app-tokens)** and the **[SDK guide](/crowdyjs/readme)** for install, configuration, and examples.
