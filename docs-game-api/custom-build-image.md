...
1. **`Dockerfile.build`** - Defines the custom build image with Node.js 18, npm, NestJS CLI, and common dependencies pre-installed
...
aws ecr create-repository \
  --repository-name crowd-rocks/web-api-build-image \
  --region us-east-2 \
...
docker buildx build --platform linux/arm64 -f Dockerfile.build -t 317700178317.dkr.ecr.us-east-2.amazonaws.com/crowd-rocks/web-api-build-image:nestjs-18-alpine .
docker push 317700178317.dkr.ecr.us-east-2.amazonaws.com/crowd-rocks/web-api-build-image:nestjs-18-alpine
```
...
- **What it does**:
  ...
  - Pushes to ECR with multiple tags: `nestjs-18-alpine`, `nestjs-18-alpine-{run_number}`, and `latest`
...
1. Check if the ECR repository exists:
   ```bash
   aws ecr describe-repositories --repository-names crowd-rocks/web-api-build-image --region us-east-2
   ```
...
- **ECR Repository**: `crowd-rocks/web-api-build-image`
- **Image Tag**: `nestjs-18-alpine`
