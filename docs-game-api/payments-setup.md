...
| `DONATION` | Increments `donations.total_amount_cents` for `(user_id, app_id)`. |
| `PROPERTY_TOKENS` | Increments `property_tokens.available` (1 token per cent). |
...
Set these in `cks-graphql-api/.env` (and mirror into the AWS Secrets Manager
...
Both providers need to point at the live `web-api` deployment. The endpoint
paths are baked into `PaymentsWebhookController`:

| Provider | URL | Method | Notes |
| --- | --- | --- | --- |
| Stripe | `POST {API_BASE_URL}/webhooks/stripe` | POST | Canonical. Stripe sends `application/json` and a `Stripe-Signature` header. The raw body is captured by `main.ts` for signature verification. |
| Stripe (legacy) | `POST {API_BASE_URL}/stripe/<event-type>` | POST | Backward-compat alias served by `StripeLegacyWebhookController`. Forwards to the same `PaymentsService.ingestWebhook(STRIPE, ...)` path. The `<event-type>` segment is ignored (the body always carries the authoritative `type`). Re-point Stripe to `/webhooks/stripe` when convenient. |
...
`{API_BASE_URL}` is whatever the public URL of your web-api environment is
(e.g. `https://dev-webapi.crowdedkingdoms.com` for dev). For local testing
...
localhost:3000/webhooks/stripe` and use the printed `whsec_...` as
...
(`4242 4242 4242 4242`). The dev DB connection in `web-api/.env` already
...
legacy path is still supported by a fallback in `PaymentsService.processWebhookEvent`:
...
## 8. Migration deploy checklist

When rolling out the new payments module to an environment for the first
time:

1. Apply migration SQL as described in **`cks-management-api/scripts/migrations/`**
   (`0001-genesis.sql` and later numbered migrations ship the payments tables instead of a
   one-off portal migration). The consolidated schema replaces legacy `stripe_events` /
   webhook handling with provider-agnostic ledger tables (`checkouts`,
   `payment_events`). Deploy migrations before enabling webhooks in production.
...
`StripeLegacyWebhookController` can be removed in a future cleanup.
