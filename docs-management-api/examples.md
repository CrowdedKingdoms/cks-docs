...
Agentic Crowdy Studio policy operations are privileged and are intentionally
not included as a generic copy-paste quickstart. Use the
[policy guide](agentic-crowdy-studio-policy) and the generated field
descriptions. The player-facing quickstart uses the normal CrowdyJS SDK.
