...
:::tip[Pass enum values as the short enumerator name in quotes, for example `CrowdyRecipient = "OwningClient"`.]
:::

:::caution[The receiver must be a real `UFUNCTION`. The `CROWDY_EVENT(Name)` macro only generates the call site. It cannot emit the `UFUNCTION`, because UnrealHeaderTool does not expand macros when it scans for reflected functions.]
:::
