...
:::caution[`UDPTimeoutSeconds` must stay above the 5 second ping interval, or the SDK will reconnect during normal traffic.]
:::
...
:::warning[`bAutoClearPersistentStateOnShutdown` only fires while UDP is still connected, so delivery is not guaranteed at shutdown. For guaranteed cleanup, call `ClearAllState` before logout instead.]
:::
...
:::caution[Without a matching profile the SDK stays inactive on that map and nothing replicates.]
:::
