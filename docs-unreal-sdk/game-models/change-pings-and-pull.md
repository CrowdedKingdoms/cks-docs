...
Notifications for one bound container are gathered for a tenth of a second before the pull, so a fight that touches a container ten times in a frame costs one read, not ten. The first pull after a container binds goes through the same window, so entities that bind together (a level loading, a wave of spawns) read their state in one bulk call rather than one read each; every entity bound to a queued container receives the pull.

## One pull in flight per container

At most one state-applying pull per container is ever out at once. A pull requested while one is already in flight, a change notification, **Refresh Game Model** (`PullNow`), a first bind, a bulk bind, or a free container's **Refresh Game Model (by Id)** (`PullDataContainer`), is held instead of sent again. When the in-flight pull lands, whether it succeeded or failed, an explicit request (Refresh Game Model, a bulk bind) is sent right away as its own follow-up; a notification or a first bind instead goes back through the usual tenth-of-a-second merge window above. A bulk bind leaves out any container whose pull is already out, since the follow-up covers it. A read that does not apply state to the cache, like the combat kit's **Get Combatant State** node, is never held: it never had a pull to overlap.

**Refresh Game Model (by Id)**'s `Succeeded` / `Failed` (and `PullDataContainer`'s `OnDone`) fires exactly once, for the read that lands after your call: either its own, if nothing was already in flight, or the follow-up your call caused. During world teardown a caller waiting on one is answered failed immediately rather than left hanging.

A pull landing after your own invoke already wrote a newer value does not roll that value back: a key an invoke wrote after the pull was sent (or, for a pull waiting in the merge window, after it was first queued) keeps the invoke's value, and if the pulled value actually differed, one follow-up pull is queued so the row re-converges a moment later. A key written by an invoke sent before the pull is not protected, since the pull is the newer of the two. This only ever covers your own invoke racing your own pull; another client's write that lands in the gap between your invoke and your next pull is not something your pull can see, and reaches you the normal way, through the next change notification.

`crowdy.gamemodel.stats` counts all of this; see the stats entry below for the exact fields and log lines.

## Your own invoke's notification

An invoke that applies its result on your client (an effect, **Call Model Function**, an invoke on a container) already put the confirmed values in your cache, so the change notification the server sends back for it would only pull what you have. The SDK counts one expected notification per invoke on that container, taken when the request is sent, and drops that many; each one expires three seconds after its own invoke. An invoke that fails, or succeeds without writing anything, gives its count back, and if its notification was already dropped by then, the container is pulled instead. A container held by two entities on this client is never counted, and its notifications are never dropped, since the result is applied to one of them only.

Because the notification carries nothing but the container id, a dropped one could have been another client's write. So every container that had a notification dropped is pulled once more within about three seconds, at most once per three seconds while you keep writing to it: another client's change reaches you at most that late, where it used to cost one pull per notification. An invoke that does not apply its result on your client (the combat kit's actions, for example) is never counted, so its notification pulls as before.
...
|---|---|---|
| **On Model Attribute Changed** (`OnModelAttributeChanged`, `FCrowdyModelAttributeChanged`) | `Target, ModelId, Attribute, OldValueJson, NewValueJson` | Once per attribute whose value changed on any apply path: a re-pull, a confirmed invoke, or a free-container refresh; actor-bound or not. A string that changes only in letter case (`alice` to `Alice`) counts as a change. Alongside, not instead of, the attribute's `CrowdyOnRep`. |
...
- The invoke-write protection only ever guards your own invoke against your own pull. It has nothing to do with another client's write landing in that same window; that one still waits for the next notification, same as always.
