...
| A door's open flag | View | `CrowdyState` boolean: a lie is a visual glitch. If the door gates progress (a lock), the lock state is a Game Model attribute | [The Two Planes](./concepts/two-planes.md#deciding-where-a-field-goes) |
...
| `CrowdyState` (+ `CrowdyOnRep`) | A `UPROPERTY` on an entity actor; the owner assigns and calls the notify itself | `qs-state` | [Crowdy State](./runtime/crowdy-state.md#mark-a-property) |
...
Three rules the examples assume: the first thing a client needs is its own pawn as a `Dynamic`, `PlayerDerived`, `LocalClient` entity spawned after **On UDP Connection Success** (`qs-player`), because the server knows a client by its actor updates and a `SpatialMulticast` reaches only clients with a fresh actor; on this release that pawn registers under a random id, since the engine possesses a runtime spawn after its `BeginPlay`, and the page says what that costs ([Quickstart](./quickstart.md#1-make-your-player-a-crowdy-entity)); a `SpatialMulticast` body runs on the caller too, so guard the call with `IsLocallyOwned()` ([RPC events](./runtime/rpc-events-cpp.md#gotchas)); and a `CrowdyOnRep` function takes no parameters, read the property for the new value ([The Two Planes](./concepts/two-planes.md#what-the-split-means-for-your-code)).
