...
**Do not build the host from a template.** This paragraph used to give one, `ck.<tier>.v7.cks-env.com`, and the template is what went stale rather than the advice: the tiers no longer share a root. Dev moved to the brand domain on 2026-08-25 and test on 2026-08-26, while production is still on the older one, so any rule you derive from one tier is wrong for another.
...
To join an existing studio org, share your **User ID** with an org owner. Find it under **Account → Profile** ([https://app.dev.crowdedkingdoms.com/account](https://app.dev.crowdedkingdoms.com/account)) or query `me { userId email }` in the [management GraphQL playground](https://ck.dev.crowdedkingdoms.com/graphql).
...
1. **`platformConfig.sharedGameApiUrl`** returns a URL on [the GraphQL playground](https://ck.dev.crowdedkingdoms.com/graphql).
